...
def test_window_capture_init() -> None:
...
def test_capture_window_not_found(mock_find_window: MagicMock) -> None:
    """ウィンドウが見つからない場合のテスト。"""
...
@patch("src.services.window_capture.WindowCapture.find_window")
def test_capture_get_rect_failed(
    mock_find_window: MagicMock, mock_get_window_rect: MagicMock
) -> None:
    """ウィンドウの矩形情報取得に失敗した場合のテスト。"""
    mock_find_window.return_value = cast(int, 12345)  # ダミーのウィンドウハンドル
