...
class PreviewCanvas:
    ...
    def _on_canvas_press(self, event: tk.Event) -> None:
        ...
        if handle_idx is not None:
            ...
            # floatをintに安全に変換
            self.roi = [int(round(img_x)), int(round(img_y)), int(round(img_x)), int(round(img_y))]
    ...
    def _on_canvas_drag(self, event: tk.Event) -> None:
        ...
        if self.dragging_handle == -1:
            ...
            # ROIを移動
            self.roi = [
                int(round(x1 + img_dx)), int(round(y1 + img_dy)),
                int(round(x2 + img_dx)), int(round(y2 + img_dy))
            ]
