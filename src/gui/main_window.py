...
class MainWindow:
    ...
    def __init__(self) -> None:
        ...
        self.capture_queue: "queue.Queue[Optional[Tuple[Image.Image, Image.Image]]]" = queue.Queue()
    ...
    def _create_ui(self) -> None:
        ...
        ttk.Label(window_frame, text="キャプチャ対象:").grid(row=0, column=0, padx=(0, 5))
        ...
        self.preview_canvas_container.grid(row=0, column=0, sticky="nsew", padx=5, pady=5)
        ...
        # 差分スコア表示
        self.diff_score_label = ttk.Label(diff_frame, textvariable=self.diff_score_var, foreground="black")
        ...
        self.diff_canvas_container.grid_rowconfigure(1, weight=0)  # コントロールフレーム用
    ...
    def _check_and_restore_window_size(self) -> None:
        ...
        if hasattr(self, 'target_width') and hasattr(self, 'target_height'):
            # サイズと位置の変更を検出
            size_changed = (abs(current_width - self.target_width) > 2 or 
                           abs(current_height - self.target_height) > 2)
            position_changed = (abs(current_x - self.target_x) > 2 or 
                              abs(current_y - self.target_y) > 2)
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    if roi is not None and self.original_frame is not None:
                        ...
                        cropped_frame = self.original_frame if self.original_frame is not None else Image.new("RGB", (100, 100), "black")
                    
                    # キューに追加
                    orig = self.original_frame if self.original_frame is not None else Image.new("RGB", (100, 100), "black")
                ...
                try:
                    ...
                    if queue_item is not None and self.detector is not None:
                        ...
                        if roi is not None and self.prev_image is not None:
                            ...
                            cropped_prev = self.prev_image if self.prev_image is not None else Image.new("RGB", cropped_frame.size, "black")
                        ...
                        self.root.after(0, lambda img=diff_img_copy: self._update_diff_view(img))
                        
                        # 差分スコア表示を更新
                        score = diff_result.score
                        self.root.after(0, lambda s=score, h=diff_result.has_difference: self._update_diff_score(s, h))
    ...
    def _init_saved_window_preview(self) -> None:
        ...
        if saved_title:
            ...
            if saved_title in available_titles:
                self.logger.info(f"保存されているウィンドウを読み込みます: {saved_title}")
