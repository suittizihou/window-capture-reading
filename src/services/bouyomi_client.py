...
class BouyomiClient:
    ...
    def __init__(self, config: Dict[str, str]) -> None:
        ...
        self.voice_type_specific = int(
            config.get("BOUYOMI_VOICE_TYPE_SPECIFIC", "0")
        )  # 0: 標準

        # 接続状態
        self._connected = False
        self._last_connect_attempt = 0
        self._connect_retry_interval = int(
            config.get("BOUYOMI_RETRY_INTERVAL", "5")
        )  # 秒
    ...
    def close(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"棒読みちゃんとの接続クローズ中にエラーが発生しました: {e}",
                exc_info=True,
            )
    ...
    def speak(self, text: str) -> bool:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.logger.error(
                    f"棒読みちゃんへの送信中にエラーが発生しました（{attempt}回目）: {e}",
                    exc_info=True,
                )
                if attempt < max_retries:
                    ...
                    self.logger.error(
                        f"最大リトライ回数({max_retries})に到達。復旧できませんでした: {text}"
                    )
        return False
    ...
    def talk(self, text: str) -> bool:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # .NETサンプルと同じ順序・型でパック
                packet = struct.pack(
                    "<hhhhhB", iCommand, iSpeed, iTone, iVolume, iVoice, bCode
                )
                ...
                self.logger.error(
                    f"棒読みちゃんバイナリ送信エラー（{attempt}回目）: {e}",
                    exc_info=True,
                )
                if attempt < max_retries:
                    self.logger.info(f"{retry_interval}秒後に再試行します...")
    ...
    def _send_command(self, command: bytes) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"棒読みちゃんにテキストを送信しました: {len(command)} バイト"
            )
            return True

        except ConnectionRefusedError:
            self.logger.warning(
                f"棒読みちゃんに接続できませんでした: {self.host}:{self.port}"
            )
            self._connected = False
            return False
            ...
            self.logger.error(
                f"棒読みちゃんへの送信中にエラーが発生しました: {e}", exc_info=True
            )
            self._connected = False
            return False
            ...
            self.logger.error(
                f"棒読みちゃんへの接続中にエラーが発生しました: {e}", exc_info=True
            )
            self._connected = False
            return False
    ...
    def _try_connect(self) -> bool:
        ...
        if (
            self._connected
            or (current_time - self._last_connect_attempt)
            ...
            remain_time = self._connect_retry_interval - (
                current_time - self._last_connect_attempt
            )
            self.logger.debug(
                f"棒読みちゃんへの再接続待機中... あと {remain_time:.1f} 秒"
            )
            return False
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"棒読みちゃんとの接続テストに成功しました: {self.host}:{self.port}"
            )
            self._connected = True
            return True
