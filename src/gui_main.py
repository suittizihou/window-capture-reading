...
def main(window_title: Optional[str] = None) -> None:
    ...
    title_label = tk.Label(
        left_frame, textvariable=title_var, font=title_font, anchor="w", width=38
    )
    title_label.pack(pady=(20, 10), padx=0, anchor="w")
    title_label.config(wraplength=600)  # タイトルが折り返されるように
    ocr_text_box = tk.Label(
        left_frame,
        textvariable=ocr_text_var,
        font=ocr_font,
        width=60,
        height=4,
        anchor="nw",
    ...
    diff_preview_frame = tk.LabelFrame(
        content_row,
        text="差分検知プレビュー",
        labelanchor="n",
        font=ocr_font,
    ...
    def draw_diff_canvas_img(img: Image.Image):
        ...
        diff_canvas_img_obj = diff_canvas.create_image(
            x_offset, y_offset, anchor="nw", image=diff_canvas_img
        )
    ...
    diff_canvas.bind(
        "<Button-4>",
        lambda e: (
            setattr(diff_canvas_zoom, "__iadd__", 0.1),
            (
                draw_diff_canvas_img(diff_preview_img)
                if diff_preview_img is not None
                else None
    ...
    diff_canvas.bind(
        "<Button-5>",
        lambda e: (
            setattr(diff_canvas_zoom, "__isub__", 0.1),
            (
                draw_diff_canvas_img(diff_preview_img)
                if diff_preview_img is not None
                else None
    ...
    def on_capture_diff() -> None:
        ...
        if frame is not None:
            ...
            # 差分検知
            try:
                has_diff, diff_ratio, diff_image = detector.detect_difference(
                    prev_image, crop_img
                )
    ...
    start_button = tk.Button(
        button_frame, text="Start", width=12, height=2, command=on_start
    )
    ...
    def draw_preview_with_roi(frame_img: Image.Image) -> None:
        ...
        preview_img_on_canvas = preview_canvas.create_image(
            x_offset, y_offset, anchor="nw", image=tk_img
        )
        ...
        roi_rect = preview_canvas.create_rectangle(
    ...
    preview_canvas.bind(
        "<Button-4>",
        lambda e: (
            zoom_var.set(min(zoom_var.get() + 0.1, zoom_slider.cget("to"))),
            on_zoom_change(),
        ),
    )
    preview_canvas.bind(
        "<Button-5>",
        lambda e: (zoom_var.set(max(zoom_var.get() - 0.1, 0.2)), on_zoom_change()),
    )
    ...
    def on_pan_drag(event):
        ...
        if pan_dragging and zoom_var.get() > min_zoom[0] + 1e-4:
            ...
            if (
                hasattr(on_zoom_change, "last_img")
                and on_zoom_change.last_img is not None
            ):
                frame_img = on_zoom_change.last_img
                img_width, img_height = frame_img.width, frame_img.height
                canvas_w = preview_canvas.winfo_width()
    ...
    def capture_loop():
        ...
        try:
            while capture_running.is_set():
                ...
                if frame is None:
                    logger.warning(
                        "キャプチャ画像が取得できませんでした（Noneが返却されました）"
                    )
                    time.sleep(0.2)
                    continue
                frame_pil = None
                ...
                try:
                    ...
                    if prev_image is None:
                        ...
                        diff_preview_img = crop_img
                        root.after(
                            0, lambda img=diff_preview_img: draw_diff_canvas_img(img)
                        )
                        continue
    ...
    def diff_detection_loop():
        ...
        while running.is_set():
            try:
                ...
                notification_cooldown = float(
                    config.get("NOTIFICATION_COOLDOWN", "2.0")
                )

                # 差分を検知
                if diff_preview_img is not None:
                    diff_result = detector.detect_difference(
                        prev_image, diff_preview_img
                    )
                    ...
                    if diff_result[0]:  # has_difference
                        ...
                        if (
                            ...
                            # 通知音を再生（設定が有効な場合）
                            if (
                                config.get("NOTIFICATION_SOUND", "true").lower()
                                == "true"
                            ):
                ...
                # インターバル中の差分検知フラグがあり、インターバルが終了した場合
                    ...
                    diff_detected_during_cooldown
                    and current_time - last_notification_time >= notification_cooldown
                ):
                    # 通知音を再生（設定が有効な場合）
    ...
    def create_settings_ui(tab, settings_list, entries_dict, config):
        ...
        for i, (key, label, input_type, *args) in enumerate(settings_list):
            ...
            if input_type == "entry":
                ...
                # スケール
                scale = ttk.Scale(
                    scale_frame, from_=min_val, to=max_val, orient="horizontal"
                )
    ...
    def show_settings_dialog() -> None:
        ...
        create_settings_ui(
            notification_tab, notification_settings, entries_dict, config
        )
